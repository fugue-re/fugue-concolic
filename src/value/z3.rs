...
struct ToAst<'c> {
    value: Option<BV<'c>>,
    ctxt: &'c mut SolverContext<'c>,
}

impl<'c> ToAst<'c> {
    fn new(ctxt: &'c mut SolverContext<'c>) -> Self {
        Self {
            value: None,
            ctxt,
        }
...
pub trait Value {
    fn ast(&self, ctxt: &mut SolverContext) -> BV;
...
impl Value for SymExpr {
    ...
    fn solve(&self, ctxt: &mut SolverContext, constraints: &[SymExpr]) -> Option<BitVec> {
        ...
        if let Expr::Val(nx) = &*nx {
            ...
            let nx = if ctxt.solver().sat() == SatResult::Sat {
    ...
    fn solve_many(&self, ctxt: &mut SolverContext, constraints: &[SymExpr]) -> Vec<BitVec> {
        ...
        if let Expr::Val(nx) = &*nx {
            ...
            let ast = nx.ast(ctxt);
            let var = BV::fresh_const(ctxt.solver().get_context(), "ivar", ast.get_width());

            ctxt.solver().assert(var._eq(&ast));

            let mut acc = Vec::with_capacity(ctxt.limit);
            while ctxt.solver().sat() == SatResult::Sat && acc.len() < ctxt.limit {
    ...
    fn minimise(&self, ctxt: &mut SolverContext, constraints: &[SymExpr]) -> Option<BitVec> {
        ...
        if let Expr::Val(nx) = &*nx {
            ...
            while high != low {
                ...
                while ctxt.solver().sat() == SatResult::Sat && high != low {
